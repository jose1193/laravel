...
<!-- INDEX SHOW CHART--> 
    <div class="max-w-4xl mx-auto mt-8 my-5 pb-5 py-5">
        ...
        <p class=" capitalize font-bold text-center text-blue-700 mt-5"><span class="text-purple-700">
          total market current year: {{date('Y')}} </span><br> ${{ number_format($sum, 2, ',', ' ') }} 
       </p>

       <p class=" capitalize font-bold text-center text-blue-700 mt-5"><span class="text-purple-700">
        total market dollar year: {{date('Y')}} </span><br> ${{ number_format($sum2, 2, ',', ' ') }} 
     </p>
  </div>
...
<script type="text/javascript">
      ...
      const data = {
        labels: labels,
        datasets: [{
          label: 'Budgets Markets Charts {{date('Y')}}',
